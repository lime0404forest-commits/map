...
- **オブジェクト** … `attr_mapping` で管理。**設計の中心**はオブジェクト定義とその属性。`type` は任意の文字列タグで、カテゴリ候補の絞り込みや既定色などに使う。**loot / landmark / colony / other** は初期版からの **旧仕様 type id**（既存データ・フォールバック互換）。新規は `object_type_ui_order` 等で別 id を定義可能。
- **カテゴリ** … `category_master` で管理。**入力の中心**は `input_type`（アイテム選択 or 数量のみ）、`show_qty`。`type`（対応種類）はオブジェクトの `type` との突き合わせ用タグ。
