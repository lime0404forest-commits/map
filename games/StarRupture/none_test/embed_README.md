...
- **`data-show-all-pins="true"`**（`embed.html` に付与済み）: フィルタ無しの既定では `other` / `point` などが非表示になり、多くのピンが見えません。テスト用では全カテゴリを表示します。
- **CSV の実URL**: `map.js` と同じフォルダの `master_data.csv`。`?debug=true` でページを開くとコンソールに `baseUrl` / `csvUrl` が表示されます。
- **CSV 形式**: 1行目はヘッダー。データ行は **6列以上**（`x`,`y` 等）が必要です。
